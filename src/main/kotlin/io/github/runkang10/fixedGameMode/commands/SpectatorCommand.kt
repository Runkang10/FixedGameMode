...
class SpectatorCommand(registry: KeyedRegistry) : BrigadierCommand {
    private val translations = registry.get<LoggedConfiguration<Translations>>("translations")


    override fun meta() = BrigadierCommandMeta("Switch gamemode to spectator.", listOf("gmsp"))

    override fun execute() = GenericGameMode.new(
        "spectator",
        Permissions.GAME_MODE.getValue(GameMode.SPECTATOR),
        GameMode.SPECTATOR,
        translations
    )
}